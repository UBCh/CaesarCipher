...
public  class Encoder {
    ...
    public static int findSymbol(char cha){
        ...
        if (index>0){ return 1;}
        else if (indexMini>0){return 2;}
        return 0;
    }

    public static int getIndex(char ch){
        switch (findSymbol(ch)) {
            case 1: return Arrays.binarySearch(alfabet,ch);
            case 2: return Arrays.binarySearch(alfabetMini,ch);
        }
        return 0;
    }
   ...
   public static char encodingCesar(char simbol){
      switch (findSymbol(simbol)) {
          case 1: return alfabet[Arrays.binarySearch(alfabet,simbol)+Key.getKey()];
          case 2: return alfabetMini[Arrays.binarySearch(alfabetMini,simbol)+Key.getKey()];
                }
       return simbol;
   }
